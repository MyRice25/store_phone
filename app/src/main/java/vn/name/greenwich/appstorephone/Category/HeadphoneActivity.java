...
import vn.name.greenwich.appstorephone.Adapter.LaptopAdapter;
...
public class HeadphoneActivity extends AppCompatActivity {
    ...
    private void loadMore() {
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                adapter_head.notifyItemInserted(productList.size() - 1);
            }
        });
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                adapter_head.notifyItemRemoved(productList.size());
                page = page + 1;
                getData(page);
        ...
        .subscribe(
                productModel -> {
                    if (productModel.isSuccess()){
                        if (adapter_head == null){
                            productList = productModel.getResult();
                            adapter_head = new HeadphoneAdapter(getApplicationContext(), productList);
                            recyclerView.setAdapter(adapter_head);
                        }else {
                            ...
                            adapter_head.notifyItemRangeInserted(position, amount_add);
                        }
    ...
    private void initView() {
        toolbar = findViewById(R.id.toolbar_headphone);
        recyclerView = findViewById(R.id.recyclerview_headphone);
